...
class LinearDeepQNetwork(nn.Module):
    def __init__(self, lr, n_actions, input_dims):
        ...
        self.H1 = nn.Linear(input_dims, 128)
        self.H2 = nn.Linear(128, n_actions)
...
#takes the current and returns list of actions
    def forward(self, state):
        layer1 = F.relu(self.H1(state))
        actions = self.H2(layer1)
        return actions
class Agent():
    ...
    def choose_action(self, state):
        if np.random.random() > self.epsilon:
            action = T.argmax(self.Q.forward(T.tensor(state, dtype=T.float).to(self.Q.device))).item()
        else:
    ...
    def learn(self, state, action, reward, state_):
        ...
        q_pred = self.Q.forward(T.tensor(state, dtype=T.float).to(self.Q.device))[T.tensor(action).to(self.Q.device)]

        q_next = self.Q.forward( T.tensor(state_, dtype=T.float).to(self.Q.device)).max()
